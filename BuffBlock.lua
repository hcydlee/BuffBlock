...
local BuffBlockMenuStrings = {
	...
	[10]= "Greater Blessing of Sanctuary",
	[11]= "Blessing of Sanctuary",
	[12]= "Blessing of the Claw",
	[13]= "Ancestral Fortitude",
	[14]= "Healing Way",
	[15]= "Inspiration",
	[16]= "Vampirism 5"
}
...
function BuffBlock_GetOption(num)
	...
	or num == 10 and BUFF_CONFIG[BB_PlayerName].GREATERBLESSINGOFSANCTUARY
	or num == 11 and BUFF_CONFIG[BB_PlayerName].BLESSINGOFSANCTUARY
	or num == 12 and BUFF_CONFIG[BB_PlayerName].BLESSINGOFTHECLAW	
	or num == 13 and BUFF_CONFIG[BB_PlayerName].ANCESTRALFORTITUDE
	or num == 14 and BUFF_CONFIG[BB_PlayerName].HEALINGWAY
	or num == 15 and BUFF_CONFIG[BB_PlayerName].INSPIRATION
	or num == 16 and BUFF_CONFIG[BB_PlayerName].VAMPIRISM5
	or nil then
...
function BuffBlock_SetOption(num)
	...
	elseif num == 12 then
		if checked then
			BUFF_CONFIG[BB_PlayerName].BLESSINGOFTHECLAW = 1
			DEFAULT_CHAT_FRAME:AddMessage("Started "..BuffBlockMenuStrings[num]);
		else
			BUFF_CONFIG[BB_PlayerName].BLESSINGOFTHECLAW = nil
			DEFAULT_CHAT_FRAME:AddMessage("Stopped "..BuffBlockMenuStrings[num]);
		end
	elseif num == 13 then
		if checked then
			BUFF_CONFIG[BB_PlayerName].ANCESTRALFORTITUDE = 1
			DEFAULT_CHAT_FRAME:AddMessage("Started "..BuffBlockMenuStrings[num]);
		else
			BUFF_CONFIG[BB_PlayerName].ANCESTRALFORTITUDE = nil
			DEFAULT_CHAT_FRAME:AddMessage("Stopped "..BuffBlockMenuStrings[num]);
		end
	elseif num == 14 then
		if checked then
			BUFF_CONFIG[BB_PlayerName].HEALINGWAY = 1
			DEFAULT_CHAT_FRAME:AddMessage("Started "..BuffBlockMenuStrings[num]);
		else
			BUFF_CONFIG[BB_PlayerName].HEALINGWAY = nil
			DEFAULT_CHAT_FRAME:AddMessage("Stopped "..BuffBlockMenuStrings[num]);
		end
	elseif num == 15 then
		if checked then
			BUFF_CONFIG[BB_PlayerName].INSPIRATION = 1
			DEFAULT_CHAT_FRAME:AddMessage("Started "..BuffBlockMenuStrings[num]);
		else
			BUFF_CONFIG[BB_PlayerName].INSPIRATION = nil
			DEFAULT_CHAT_FRAME:AddMessage("Stopped "..BuffBlockMenuStrings[num]);
		end
	elseif num == 16 then
		if checked then
			BUFF_CONFIG[BB_PlayerName].VAMPIRISM5 = 1
			DEFAULT_CHAT_FRAME:AddMessage("Started "..BuffBlockMenuStrings[num]);
		else
			BUFF_CONFIG[BB_PlayerName].VAMPIRISM5 = nil
			DEFAULT_CHAT_FRAME:AddMessage("Stopped "..BuffBlockMenuStrings[num]);
		end
	end
end
...
function Kill_Buffs()
	...
	while not (GetPlayerBuff(i, "HELPFUL") == -1) do
		...
		if BUFF_CONFIG[BB_PlayerName].GREATERBLESSINGOFSANCTUARY then
			if (string.find(texture,"Spell_Holy_GreaterBlessingofSanctuary")) then
				CancelPlayerBuff(buffIndex);
		...
		if BUFF_CONFIG[BB_PlayerName].BLESSINGOFSANCTUARY then
			if (string.find(texture,"Spell_Nature_LightningShield")) then
				CancelPlayerBuff(buffIndex);
				DEFAULT_CHAT_FRAME:AddMessage("Blocked "..BuffBlockMenuStrings[11], 1, 1, 0.5);
			end
		end
		if BUFF_CONFIG[BB_PlayerName]. BLESSINGOFTHECLAW then
			if (string.find(texture,"Spell_Holy_BlessingOfAgility")) then
				CancelPlayerBuff(buffIndex);
				DEFAULT_CHAT_FRAME:AddMessage("Blocked "..BuffBlockMenuStrings[12], 1, 1, 0.5);
			end
		end
		if BUFF_CONFIG[BB_PlayerName]. ANCESTRALFORTITUDE then
			if (string.find(texture,"Spell_Nature_UndyingStrength")) then
				CancelPlayerBuff(buffIndex);
				DEFAULT_CHAT_FRAME:AddMessage("Blocked "..BuffBlockMenuStrings[13], 1, 1, 0.5);
			end
		end
		if BUFF_CONFIG[BB_PlayerName]. HEALINGWAY then
			if (string.find(texture,"Spell_Nature_HealingWay")) then
				CancelPlayerBuff(buffIndex);
				DEFAULT_CHAT_FRAME:AddMessage("Blocked "..BuffBlockMenuStrings[14], 1, 1, 0.5);
			end
		end
		if BUFF_CONFIG[BB_PlayerName]. INSPIRATION then
			if (string.find(texture,"INV_Shield_06")) then
				CancelPlayerBuff(buffIndex);
				DEFAULT_CHAT_FRAME:AddMessage("Blocked "..BuffBlockMenuStrings[15], 1, 1, 0.5);
			end
		end
		if BUFF_CONFIG[BB_PlayerName]. VAMPIRISM5 then
			if (string.find(texture,"Spell_Shadow_UnsummonBuilding")) then
				CancelPlayerBuff(buffIndex);
				DEFAULT_CHAT_FRAME:AddMessage("Blocked "..BuffBlockMenuStrings[16], 1, 1, 0.5);
			end
		end
